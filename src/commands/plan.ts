...
import { runPlanList } from "./plan-list.ts";
	...
	plan
		...
		.action(
			async (opts: {
				...
				await runPlanList("plan list", opts, Boolean(opts.json));
			},
		);

	plan
		.command("ready")
		.description("List plans that aren't done (status draft|approved|active)")
		.option("--seed <id>", "Filter by parent seed id")
		.option("--outcome <outcome>", "Filter by outcome (success|partial|failure)")
		.option("--template <name>", "Filter by template name")
		.option("--json", "Output as JSON")
		.action(
			async (opts: { seed?: string; outcome?: string; template?: string; json?: boolean }) => {
				await runPlanList("plan ready", opts, Boolean(opts.json), true);
			},
		);
